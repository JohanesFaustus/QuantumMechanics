...
Now to find the constraining force, we must use the Lagrange multiplier.
From the constraint it is obvious that only radial constraint present.
As such,
\begin{align*}
    \frac{\partial \mathcal{L}}{\partial\rho}+\lambda\frac{\partial f}{\partial\rho}   & =\frac{d}{dt}\frac{\partial \mathcal{L}}{\partial \dot{\rho}} \\
    \frac{\partial \mathcal{L}}{\partial \phi}+\lambda\frac{\partial f}{\partial \phi} & =\frac{d}{dt}\frac{\partial \mathcal{L}}{\partial \dot{\phi}} \\
    \frac{\partial \mathcal{L}}{\partial z}+\lambda\frac{\partial f}{\partial z}       & =\frac{d}{dt}\frac{\partial \mathcal{L}}{\partial \dot{z}}    \\
    f(r)                                                                               & = r-R=0
\end{align*}
where the Lagrangian is of course unmodified
\begin{equation*}
    \mathcal{L}=\frac{1}{2}m(\dot{\rho}^2 +\rho^2\dot{\phi}^2+\dot{z}^2)-\frac{1}{2}k(\rho^2+z^2)
\end{equation*}
We only need the $\rho$ equation and $f(r)$ to determine the constraint
\begin{align*}
    m\rho \dot{\phi}^2 + k\rho +\lambda & =  \frac{d }{dt }m \dot{\rho} = m \ddot{\rho} \\
    \rho - R                            & =  0
\end{align*}
The constraint implies $\dot{\rho}=\ddot{\rho}=0$, so the constraining force is
\begin{equation*}
    \lambda = R(k-m \dot{\phi}^2)
\end{equation*}
This constraint is the normal force $N$.
The requirement that the particle is constrained within $\rho=R$ is $N \geq 0$ or
\begin{equation*}
    k \geq \frac{L^2    }{mR^4}
\end{equation*}

Comparison with Newton method: the acceleration in cylindrical components is
\begin{equation*}
    \mathbf{a} = (\ddot{\rho} - \rho \dot{\phi}^2) \hat{\rho} + (\rho \ddot{\phi} + 2 \dot{\rho} \dot{\phi}) \hat{\phi} + \ddot{z} \hat{z}
\end{equation*}
With the constraint, this reads
\begin{equation*}
    \mathbf{a} = \rho \dot{\phi}^2 \hat{\rho} + \rho \ddot{\phi} \hat{\phi} + \ddot{z} \hat{z}
\end{equation*}
The force also reads
\begin{equation*}
    \mathbf{F}_{\mathrm{spring}} = -kr = -k(R\hat{\rho} + z\hat{\mathbf{z}})
\end{equation*}
Newton law state
\begin{equation*}
    m \mathbf{a} = \mathbf{F}_{\mathrm{spring}} + N \hat{\rho}
\end{equation*}
Thus the normal force reads
\begin{equation*}
    N = R(k- \ddot{\phi}^2)
\end{equation*}
For $N \geq 0$, this also reads
\begin{equation*}
    k \geq m \dot{\phi}^2 \geq \frac{L^2 }{mR^4}
\end{equation*}
...
\begin{align*}
    ...
    \ddot{q}_1
               & =
    \frac{g\sin\alpha (M+m)}{M + m\sin^{2}\alpha}
\end{align*}
...
\begin{equation*}
    t=\left[\frac{2l (M + m\sin^{2}\alpha)}{g\sin\alpha (M+m)}\right]^{1/2}
\end{equation*}
...
Now lets us try to use the Newtonian approach.
Let $q_{1}$ be the block displacement down the slope and $q_{2}$ the horizontal displacement of the wedge.
The block horizontal coordinate is
\begin{equation*}
    \dot{x} = q_{2} + q_{1}\cos\alpha,
\end{equation*}
so that
\begin{equation*}
    \ddot{x} = \ddot{q}_2 + \ddot{q}_1\cos\alpha.
\end{equation*}
Conservation of horizontal momentum gives
\begin{equation*}
    M \ddot{q}_2 + m\left(\ddot{q_2}+\ddot{q}_1\cos\alpha\right) = 0,
\end{equation*}
hence
\begin{equation*}
...
\begin{equation*}
    \ddot{q}_2  =-\frac{m}{M+m}\ddot{q}_1\cos\alpha
\end{equation*}
In the noninertial frame of the wedge the block experiences gravity and the inertial force $-m \ddot{q}_2$ along the horizontal.
Projecting along the slope yields
\begin{equation*}
    m \ddot{q}_1 = m g\sin\alpha - m \ddot{q}_2\cos\alpha.
\end{equation*}
Substituting $\ddot{q}_2$ gives
\begin{equation*}
    m\ddot{q}_1 = m g\sin\alpha + \frac{m^{2}\cos^{2}\alpha}{M+m}\ddot{q}_1.
\end{equation*}
Dividing by $m$ and solving for the acceleration,
\begin{align*}
    \ddot{q}_1 \left( 1 - \frac{m\cos^{2}\alpha}{M+m} \right) & =  g\sin\alpha \\
                                                              & =
    \frac{g\sin\alpha}{1 - \frac{m\cos^{2}\alpha}{M+m}}                        \\
    \ddot{q}_1
                                                              & =
    \frac{g\sin\alpha (M+m)}{M + m\sin^{2}\alpha}.
\end{align*}
If the block travels a distance l from rest, the time required is
\begin{equation*}
    t = \sqrt{\frac{2l}{\ddot q_{1}}}=\left[\frac{2l (M + m\sin^{2}\alpha)}{g\sin\alpha (M+m)}\right]^{1/2}
\end{equation*}

...
\begin{equation*}
    \rho^{A} \equiv \mathrm{Tr}_{{A}'}(\rho) = \sum_{n} \left( \mathbf{1 }_A\otimes\bra{n_{A'}} \right) \rho \left( \mathbf{1 }_A\otimes\ket{n_{A'}} \right)
\end{equation*}
where $\rho$ denote the density operator acting on $\mathcal{H}$ and $\left\{ \ket{n}_{A'}\right\} $ is a complete basis for subsystem $A'$, and it only lives on said subspace, thus the identity tensor product.

...
This solution represent plane wave with momentum $\hbar k$ toward$x$ and one with momentum $-\hbar k$ negative $-x$.

In general, we can obtain the expression for position-space representation of the plane wave by considering the eigenstate of the momentum operator
\begin{equation*}
  \mathbf{p }\ket{\mathbf{k }}  = \hbar \mathbf{k }\ket{\mathbf{k}}
\end{equation*} 
Then project it in position-space
\begin{align*}
    \braket{\mathbf{r }|\mathbf{p } | \mathbf{k }} 
  & =  \hbar \mathbf{k} \braket{\mathbf{r } | \mathbf{k}}\\
  -i \hbar \nabla \psi(\mathbf{r }) 
  & =  \hbar \mathbf{k }\psi(\mathbf{r })\\
  \psi(\mathbf{r})
  & = C e ^{i \mathbf{k }\cdot \mathbf{r}}
\end{align*}
To normalize, we use the momentum–space Dirac delta normalization
\begin{equation*}
    \langle \mathbf{k}|\mathbf{k}'\rangle = \int d^3r\,\langle \mathbf{k}|\mathbf{r}\rangle\langle \mathbf{r}|\mathbf{k}'\rangle
= |C|^2\int d^3r \; e^{i(\mathbf{k}'-\mathbf{k})\cdot\mathbf{r}}=
(2\pi)^{3/2}\delta^{(3)}({\mathbf{k }-\mathbf{k}'})
\end{equation*}
With $\mathbf{k }=\mathbf{k}'$, the normalization constant is 
\begin{equation*}
  C = \frac{1 }{(2\pi) ^{3/2}}
\end{equation*}

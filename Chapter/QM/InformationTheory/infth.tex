...
\begin{equation*}
  S(I, J) = - \sum_{i \in I,\, j \in J} p(i,j) \log_2 p(i,j)
\end{equation*}
...
From this, it is likely that the name ``mutual information'' was given.
\begin{align*}
  S(I : J) &= S(I) + S(J) - S(I, J) \\
...
\begin{align*}
  S(I : J) &= S(I) + S(J) - S(I, J) \\
&= - \sum_{i=1}^{N} p_i \log_2 p_i - \sum_{j=1}^{N} q_j \log_2 q_j + \sum_{i,j=1}^{N} p(i,j) \log_2 p(i,j) \\
&= - \sum_{ij}^{N} p(i,j) \log_2 p_i - \sum_{ij}^{N} p(i,j) \log_2 q_j + \sum_{i,j=1}^{N} p(i,j) \log_2 p(i,j) \\
S(I : J) 
&= \sum_{i,j=1}^{N} p(i,j) \log_2 \frac{p(i,j)}{p_i q_j}
\end{align*}

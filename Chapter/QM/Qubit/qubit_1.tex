...
\begin{equation*}
  \mathcal{E}_\text{coh}(0) = 2B|\beta|^2 \qquad
  \mathcal{E}_\text{mix}(0) =\mathcal{E} =2B(2q-1) 
\end{equation*}
...
and
\begin{equation*}
  \mathcal{E}_\text{mix} (t) = 2[2q |f_N|^2 - 1]
\end{equation*}


\subsubsection{Derivation: general ergotropy.}
The general expression for our state is 
\begin{equation*}
  \rho = \begin{bmatrix}
        \rho_{00}&\rho_{01}\\
        \rho_{10}&\rho_{11}
  \end{bmatrix}
\end{equation*}
The eigenvalue of this state is
\begin{align*}
...
For the case of population inversion, the derivation is very simple
\begin{align*}
...
\begin{align*}
  ...
  & = B \left[  1 - 2 \rho_{11} + \sqrt{1-4 \det \rho}  \right] \\
...
\subsubsection{Derivation: ergotropy for another mixed state.}
...
\begin{align*}
  \rho
& =
p\ket{\psi_1}\bra{\psi_1}
+
(1-p)\ket{\psi_2}\bra{\psi_2}\\
& = 
p
\begin{bmatrix}
\cos^2\frac{\theta_1}{2}
&
e^{-i\phi_1}\cos\frac{\theta_1}{2}\sin\frac{\theta_1}{2}\\\\
e^{i\phi_1}\cos\frac{\theta_1}{2}\sin\frac{\theta_1}{2}
&
\sin^2\frac{\theta_1}{2}
\end{bmatrix}\\
...
\cos^2\frac{\theta_2}{2}
&
e^{-i\phi_2}\cos\frac{\theta_2}{2}\sin\frac{\theta_2}{2}\\\\
e^{i\phi_2}\cos\frac{\theta_2}{2}\sin\frac{\theta_2}{2}
&
\sin^2\frac{\theta_2}{2}
\end{bmatrix}\\
\rho & =
\begin{bmatrix}
p\cos^2\frac{\theta_1}{2}
+(1-p)\cos^2\frac{\theta_2}{2}
&
p e^{-i\phi_1}\frac{\sin \theta_1}{2}
+(1-p)e^{-i\phi_2}\frac{\sin \theta_2}{2}
\\\\
p e^{i\phi_1}\frac{\sin \theta_1}{2}
+(1-p)e^{i\phi_2}\frac{\sin \theta_2}{2}
&
p\sin^2\frac{\theta_1}{2}
+(1-p)\sin^2\frac{\theta_2}{2}
\end{bmatrix}
\end{align*}
We impose then the constraint of $\theta_1=-\theta_2=\theta$ and $\phi_1=-\phi_2=\phi$
\begin{align*}
  \rho & = 
  \begin{bmatrix}
    \cos ^2 \theta/2 & \frac{\sin \theta  }{2} \left[p(e^{-i\phi}+e^{i\phi}) -e^{i\phi}  \right] \\
   \frac{\sin \theta  }{2} \left[p(e^{-i\phi} + e^{i\phi} ) - e^{-i\phi}  \right] & \sin ^2 \theta /2
  \end{bmatrix}\\
    \rho & = 
    \begin{bmatrix}
    \cos ^2 \theta/2 &   \frac{\sin \theta  }{2} \left[2p \cos \phi - e^{i\phi}  \right]\\
      \frac{\sin \theta  }{2} \left[2p \cos\phi - e^{-i\phi}  \right] &  \sin ^2 \theta /2
  \end{bmatrix}
\end{align*}
From 
\begin{equation*}
  \rho_\text{out}(t) = 
  \begin{bmatrix}
    1 -\rho_{11}(0)|f_N(t)|^2 & \rho_{01}(0)f_N(t)\\
    [\rho_{01}(0)f_N(t)]^* & \rho_{11}(0)|f_N(t)|^2
  \end{bmatrix}
\end{equation*}
...
Let us find first the initial state determinant
\begin{align*}
    \det\rho
& = 
...
-
\frac{\sin^2\theta}{4}
\left|p(e^{-i\phi}+e^{i\phi})-e^{i\phi}\right|^2\\
& = 
\frac{\sin^2\theta}{4}
-\frac{\sin^2\theta}{4}
\left[
4p^2\cos^2\phi
-2p\cos\phi(e^{i\phi}+e^{-i\phi})
+1
\right]\\
= &
\frac{\sin^2\theta}{4}
-\frac{\sin^2\theta}{4}
\left[
4p^2\cos^2\phi-4p\cos^2\phi+1
\right]\\
& = 
\sin ^2\theta(p \cos ^2\phi - p^2 \cos \phi)\\
\det \rho & = 
p(1-p)\sin ^2\theta \cos^2 \phi
\end{align*}
And the evolved determinant
\begin{align*}
  \det \rho(t) & = (1-\rho_{11}|f_N|^2)\rho_{11}|f_N|^2 - |\rho_{01}f_N|^2\\
  & = |f_N|^2 (\rho_{11}-\rho_{11}^2|f_N|^2-|\rho_{01}|^2)\\
  & = |f_N|^2(\rho_{11} - \rho_{11}-|\rho_{01}|^2 + \rho_{11}^2 - \rho_{11}^2|f_N|^2)\\
  & = |f_N|^2[\det \rho(0)+\rho_{11}^2(1-|f_N|^2)]\\
 \det \rho^{(N)}(t) & = |f_N|^2 \left[ p(1-p)\sin ^2\theta \cos \phi + \sin ^4 \frac{\theta }{2 }(1-|f_N|^2) \right] 
\end{align*}
Therefore the ergotropy is 
\begin{align*}
  \mathcal{E} [\rho^{(1)}_\text{mix}] & =  B \left( 2\rho_{11}^{(1)}- 1 + \sqrt{1-4 \det \rho^{(1)}} \right) \\
  & = B \left( 2\sin^2 \frac{\theta }{2} - 1 + \sqrt{1 - 4p(1-p)\sin ^2\theta \cos \phi}  \right) 
\end{align*}
and
\begin{multline*}
      \mathcal{E} [\rho^{(N)}_\text{mix}]    = B \bigg( 2\sin^2 \frac{\theta }{2} |f_N|^2 - 1 
  \\  \qquad+  \sqrt{1 - 4|f_N|^2 \left[ p(1-p)\sin ^2\theta \cos \phi + \sin ^4 \frac{\theta }{2 }(1-|f_N|^2) \right]}  \bigg)    
\end{multline*}

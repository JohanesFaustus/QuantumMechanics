...
\end{align*}
The stationarity of $E_f$ thus implies that $\rho(\tau )$ should commute with $H$.
And, the unitary transformation enforce that $\rho(\tau)$ have the same eigenvalues as $\rho_0$.
...
First we set the ground state as zero, that is $H \ket{\mathbf{0}} = E_0 \ket{\mathbf{0}} = 0$, or 
\begin{equation*}
  H = \begin{bmatrix}
    0&0\\
    0&2B
...
\begin{equation*}
  \mathcal{E}_\text{coh}(0) = |\beta|^2 \bra{\mathbf{1}} H \ket{\mathbf{1} }\qquad
  \mathcal{E}_\text{mix}(0) =\mathcal{E} =(2q-1) \braket{\mathbf{1 } | H| \mathbf{1}}  
\end{equation*}
with $\braket{\mathbf{1 } | H| \mathbf{1}}  $ as $2B$ in the shifted Hamiltonian.
...
\subsubsection{Derivation: ergotropy of pure and mixed state}
First consider the initial encoded ergotropy.
The passive state for the pure state is simply the ground state, that is 
\begin{equation*}
  H \ket{\mathbf{0}} = E_0 \ket{\mathbf{0}} = 0
\end{equation*}
while the initial energy is 
\begin{equation*}
...
\begin{equation*}
  ...
  \end{bmatrix}
\end{equation*}
Hence, the initial energy for pure and mixed state respectively are 
\begin{equation*}
  E_{\text{coh}_i} = 2B |\beta|^2 |f_N|^2,\qquad
  E_{\text{mix}_i} = 2B (1-q) |f_N|^2
\end{equation*}
...
The characteristic equation for $\rho$ as $2 \times 2$ matrix is 
\begin{align*}
  \det (\rho -\lambda I) & =  \det \begin{pmatrix}a-\lambda & b\\ c & d-\lambda\end{pmatrix}\\
...
\end{equation*}
and, we have the passive energy as
\begin{equation*}
  E_p = \lambda_+ H_{00} + \lambda_- H_{11} = 2B\lambda_- 
\end{equation*}
Using the initial and passive energy, we find that the ergotropy for coherent state is 
\begin{equation*}
  \mathcal{E}_\text{coh} (t) = 2|f_N(t)|^2\sin^2\left(\frac{\theta}{2}\right)-1+\sqrt{1+4\sin^4\left(\frac{\theta}{2}\right)|f_N(t)|^2\left(|f_N(t)|^2-1\right)}
\end{equation*}
after substituting $(\alpha,\beta)$ parameters and normalizing by $2B$.
As for mixed state, the ergotropy is stored in population inversion.
Assuming that the higher state population $(1-q)|f_N|^2>0.5$, the passive energy after population inversion is 
\begin{equation*}
E_p = 2B[q + (1-q)(1-|f_N|^2)]
\end{equation*}
and the ergotropy of 
\begin{equation*}
  \mathcal{E}_\text{mix} (t) = 2[2q |f_N|^2 - 1]
\end{equation*}

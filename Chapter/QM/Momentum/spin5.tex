...
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{\input{../../../Rss/QM/Momentum/plot_FvsN.pgf}}
    \resizebox{\textwidth}{!}{\input{../../../Rss/QM/Momentum/plot_FvsN_tmore.pgf}}
...
\begin{figure}[t]
    \centering
    \resizebox{\textwidth}{!}{\input{../../../Rss/QM/Momentum/plot_F_vs_k.pgf}}
...
\begin{figure}
    ...
    \begin{tikzpicture}[x=1.4cm,y=1cm]
        ...
        \draw[thick] (0,0) -- (6,0);
        % index
        \node[anchor=east] at (-0.3, 0.5) {$J_{j,j+1} =$};
        \node[anchor=east] at (-0.3, 1.5) {$J_{j,j+2} =$};
        \node[anchor=east] at (-0.3, 2.5) {$\vdots$};
        \node[anchor=east] at (-0.3, 3.5) {$J_{j,j+6}= $};
        \node[anchor=east] at (-0.3, -0.75) {$j =$};
        % coupling str
        \node at (0.5, 0.5) {$\alpha$};
        \node at (1.5, 0.5) {$\alpha$};
        \node at (2.5, 0.5) {$J_{34}$};
        \node at (3.5, 0.5) {$J_{45}$};
        \node at (4.5, 0.5) {$\alpha$};
        \node at (5.5, 0.5) {$\alpha$};
        % coupling str 2
        \node at (1, 1.5) {$J_{13}$};
        \node at (2, 1.5) {$J_{24}$};
        \node at (3, 1.5) {$J_{35}$};
        ...
        \node[anchor=north] at (1, -0.5) {$2$};
        \node[anchor=north] at (2, -0.5) {$3$};
        \node[anchor=north] at (3, -0.5) {$4$};
        \node[anchor=north] at (4, -0.5) {$5$};
        \node[anchor=north] at (5, -0.5) {$6$};
        \node[anchor=north] at (6, -0.5) {$7$};
        % site
        \node[anchor=north] at (0, -1.5) {$\mathbf{\ket{1}}$};
        \node[anchor=north] at (6, -1.5) {$\mathbf{\ket{N}}$};
    \end{tikzpicture}
    \caption*{Figure: Dual-Hole coupling}
\end{figure}
...
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{\input{../../../Rss/QM/Momentum/plot_FvsN_long.pgf}}
...
\subsection{Optimization For XXX Hamiltonian: Engineered Coupling}
...
% For example, the input site can be labeled as $\mathbf{\ket{1}}  \to \ket{-(N-1)/2}$ and the output as $\ket{\mathbf{N}} \to \ket{+(N-1)/2}$.
...
\begin{equation*}
    S_{+}   \ket{S, m} =  \frac{1 }{2 }\sqrt{j(N \mp j)}   \ket{S, m +  1}\qquad
...
\begin{equation*}
    \braket{j+1 | H|j} = \frac{1 }{2 }\sqrt{j(N-j)}
\end{equation*}
In any case, we have the matrix element
\begin{equation*}
    H  = -\frac{\lambda}{4}\begin{pmatrix}
        0      & J_{1}      & 0      & \cdots \\
        J_1      & 0      & J_2      & \cdots \\
        0      & J_2      & 0      & \cdots \\
...
Next we can write the propator as 
\begin{equation*}

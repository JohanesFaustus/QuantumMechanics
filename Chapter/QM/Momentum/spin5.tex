...
\begin{equation*}
  f(t) = \sum_{j=1}^{N} e^{-i E_j t}\braket{\mathbf{r}|\lambda_j }\braket{\lambda_j |\mathbf{s} }
\end{equation*}
Defining the projection of eigenstates $\ket{\lambda_j}$ to sender as $|\sigma_j| e ^{\phi_j}$ and to receiver as $|\rho_j| e ^{\psi_j}$, we have
\begin{equation*}
...
\begin{align*}
    ...
    & =  \sum_{j } \braket{\mathbf{s }  | \lambda_j } \braket{\lambda_j  | \mathbf{s}}
    = \sum_{j } |\sigma_j|^2 = 1\\
    \braket{\mathbf{r} | \mathbf{r}} 
    & =  \sum_{j } \braket{\mathbf{r }  | \lambda_j } \braket{\lambda_j  | \mathbf{r}}
    = \sum_{j } |\rho_j|^2 = 1 
\end{align*}
For fixed $j$, we can also expand in terms of other one-excitation state
\begin{equation*}
    \braket{\lambda_j  | \lambda_j } 
    = \sum_{ \mathbf{i} }  \braket{\lambda_j  | \mathbf{i } } \braket{\mathbf{i }  | \lambda_j }
    = \sum_{\mathbf{i} \neq (\mathbf{s},\mathbf{r})}  |\braket{\lambda_j  | \mathbf{i }}|^2 + |\sigma_j|^2 + |\rho_j|^2 = 1
\end{equation*}
...
\begin{equation*}
    \ket{\lambda_\pm} = \frac{1 }{\sqrt{2}} \left( \ket{\mathbf{s }} \pm \ket{\mathbf{r}} \right) 
\end{equation*}
In this case, we have unitary amplitude
\begin{align*}
    f(t) & =  
    e ^{-i E_+ t} \braket{\mathbf{r } |\lambda_+ } \braket{\lambda_+  | \mathbf{s}} +
    e ^{-i E_- t} \braket{\mathbf{r } |\lambda_- } \braket{\lambda_-  | \mathbf{s}}\\
...
Dual hole method is then applied by setting the coupling constants $J_{2,i}$ and $J_{N-1,i}$.

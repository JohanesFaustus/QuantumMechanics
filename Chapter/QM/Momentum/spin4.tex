...
\begin{document}
\subsection{Heisenberg Hamiltonian Periodic boundary}
...
\end{equation*}
or equivalently using  $    \boldsymbol{\sigma}_i\cdot\boldsymbol{\sigma}_{i+1}=2P_{i,i+1}-1$
\begin{equation*}
...
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{\input{../../../Rss/QM/Momentum/fig.pgf}}
    \caption*{Figure: The bar plot shows the maximum fidelity $F$ of quantum communication in a time interval [0, 4000/J] as a function of the chain length $N$ from 2 to 80.}
\end{figure}
...
Further analytic consideration can be done with writing the amplitude back in terms of momentum $k = \pi(m-1)/N$
\begin{align*}
    g_N 
    & = \sum_{m } a_m ^ 2 \exp \left[ -i4Jt(1 - \cos k) \right] \cos (k(N-1/2)) \cos (k/2)\\
    & = \frac{1 }{2 }\sum_{m } a_m ^ 2 \exp \left[ -i4Jt(1 - \cos k) \right] \left[ \cos kN \cos k/2 - \sin kN \sin k/2 \right]  \cos (k/2)\\
    & = \frac{1 }{2 }\sum_{m } a_m ^ 2\cos k/2 \exp \left[ -i4Jt(1 - \cos k) \right](-1)^{m-1} \\
    & = \frac{1 }{2 }\sum_{m } a_m ^ 2\cos k/2 \exp \left[i \left\{\pi(m-1) - 4t (1-\cos k)  \right\}  \right]\\
    & = \frac{1 }{2 }\sum_{m } a_m ^ 2\cos k/2 \exp \left[i \left\{Nk - 4t (1-\cos k)  \right\}  \right]
\end{align*}
where $kN = \pi (m-1)$ and using the condition $r=N$, $s=1$. 
With $\Delta k = \pi /N$, we can approximate the sum as integral 
\begin{equation*}
    g_N = \frac{N }{2 \pi } \int_m dk\;  a_m ^ 2\cos^2 (k/2) \exp \left[i \left\{Nk - 4t (1-\cos k)  \right\}  \right]
\end{equation*}
Considering that $a_m$ depend on the value of $m$, we can split the integral as 
\begin{multline*}
    g_N = \frac{1 }{2 \pi } \exp \left[i \left\{Nk - 4t (1-\cos k)  \right\}  \right] \\
    + \int_{k_2} ^{k_N} dk\;  a_m ^ 2\cos^2 (k/2) \exp \left[i \left\{Nk - 4Jt (1-\cos k)  \right\}  \right]    
\end{multline*}
with $k_\text{2} = \pi/N$ and $k_N = \pi(N-1)/N$.
Under large $N$, the limit can be approximated as 
\begin{equation*}
    g_N = \frac{1 }{2 \pi }     + \frac{1 }{\pi} \int_{0} ^{\pi} dk\; \cos^2 (k/2) \exp \left[i \left\{Nk - 4Jt (1-\cos k)  \right\}  \right]    
\end{equation*}
...
This can be rigorously proved using repeated integration by parts.
We determine this $k_0$ by 
\begin{equation*}
    \frac{d\Phi(k )}{dk} = 0 \implies \sin k_0 = \frac{N }{4Jt}
\end{equation*}
Expanding the phase around $k_0$
\begin{align*}
    \Phi (k ) & =  \Phi(k_0) + \Phi'(k_0)(k-k_0) + \tfrac{1}{2}\Phi''(k_0)(k-k_0)^2 + \dots\\
    \phi(k) & \approx \Phi(k_0) - 2t\cos k_0(k-k_0)^2  
\end{align*}
with $\Phi'(k_0) = 0$.
As such, the integral term reads
\begin{equation*}
    g_N = \frac{1 }{2 \pi }     + \frac{1 }{\pi} e ^{i\Phi(k_0)}\cos ^2 (k_0/2) \int_{0} ^{\pi} dk\;  \exp \left[-i  2t\cos k_0(k-k_0)^2    \right]
\end{equation*}
Next make the substitution $q= k - k_0$
\begin{equation*}
    g_N = \frac{1 }{2 \pi }     + \frac{1 }{\pi} e ^{i\Phi(k_0)}\cos ^2 (k_0/2) \int_{k_0} ^{\pi - k_0} dk\;  \exp \left[-i  2t\cos k_0q^2    \right]
\end{equation*}
We can also expand the integration limit 
\begin{equation*}
    g_N = \frac{1 }{2 \pi }     + \frac{1 }{\pi} e ^{i\Phi(k_0)}\cos ^2 (k_0/2) \int_{-\infty} ^{\infty} dk\;  \exp \left[-i  2t\cos k_0q^2    \right]
\end{equation*}
since it oscillates increasingly rapidly away from $k_0$, producing destructive interference.
Evaluating the last Fresnel-type integral
\begin{equation*}
    g_N = \frac{1 }{2 \pi }     + \frac{1 }{\pi} e ^{i\Phi(k_0)}\cos ^2 (k_0/2) \sqrt{\frac{\pi }{2t |\cos k_0|} } \exp \left[ -i \frac{\pi }{4 } \mathrm{sgn } (\cos k_0) \right]     
\end{equation*}
or
\begin{multline*}
    g_N = \frac{1 }{2 \pi }     + \frac{1 }{\pi} \cos ^2 (k_0/2) \sqrt{\frac{\pi }{2t |\cos k_0|} } \\
    \times \exp \left[ iNk_0 -i4Jt (1-\cos k_0)-i \frac{\pi }{4 } \mathrm{sgn } (\cos k_0) \right]     
\end{multline*}
with $k_0 = \arcsin (N/4Jt)$.
...
\begin{align*}
    H \ket{0} 
    & =  -2J \sum_{i  = 1}^{N-1}  (2P_{i,i+1}-1)\ket{0}   - B\sigma_3 \ket{0} \\

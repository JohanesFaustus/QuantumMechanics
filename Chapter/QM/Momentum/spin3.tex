...
Do note the $a ^\dagger$ here is defined as abstract operator that create excitations state, regardless of how the ground state $\ket{0}$ is defined, whether all up or all down.

\subsubsection{One-dimensional case: periodic boundary.}
...
\subsubsection{One dimensional case: open chain.}
Also suppose we add external non modulation field
\begin{equation*}
    H = -J\sum_{j=1}^{N}\boldsymbol{\sigma}_j\cdot\boldsymbol{\sigma}_{j+1} - \sum_{i } ^N B \sigma_{3i} 
\end{equation*}
Here we use common convention of all down ground state $\ket{0}$ and one magnon state $\ket{\mathbf{j }}$ as the state at which the spin at the $j$th site has been flipped to the $\ket{1}$ state.
...
\begin{align*}
    H_1 \ket{1} 
    & =   -J \boldsymbol{\sigma}_{1} \cdot \sigma_{2} \ket{1}    -J \sum_{i \neq i}  \boldsymbol{\sigma}_{i} \cdot \sigma_{i+1} \ket{1} \\
    & =   -J (2P_{1,2} -1)\ket{1}    -J (N-1) \ket{1} \\
    & =  -2 J \ket{2 } -J (N-2) \ket{1}
\end{align*}
with $H_1$ as the Heisenberg Hamiltonian at the first term.
For the other end 
\begin{align*}
    H_1 \ket{N} 
    & =   -J \boldsymbol{\sigma}_{N-1} \cdot \sigma_{N} \ket{N}    -J \sum_{i \neq N-1}  \boldsymbol{\sigma}_{i} \cdot \sigma_{i+1} \ket{N} \\
    & =   -J (2P_{N-1,N} -1)\ket{N}    -J (N-1) \ket{N} \\
    & = -2 J \ket{N-1 } -J (N-2) \ket{N}
\end{align*}
while for the interior is the same as the periodic boundary case 
\begin{equation*}
    H_1 \ket{j} = E_0 \ket{j} + 2J (2 \ket{j }- \ket{j-1} - \ket{j+1})
\end{equation*} 
...
\begin{align*}
    H_1 \ket{\tilde{m}  } & = 
    a_1 ^{m} H_1 \ket{1} + \sum_{j=2 }^{N-1} a_j ^{m } H_1\ket{\mathbf{j}} + a_N ^{m} H_1 \ket{N}\\
    & = -a_1 ^{m} \left[ 2J \ket{2 } + J (N-2) \ket{1} \right] - a_N ^{m} \left[ 2J \ket{N-1} + J(N-2) \ket{N}\right]  \\
    & \qquad- \sum_{j=2 } ^{N-1} a_j ^{m} \left[  E_0 \ket{j} + 2J (2 \ket{j }- \ket{j-1} - \ket{j+1}) \right]  
\end{align*}
From this we obtain the cases for the interior and the boundary 
\begin{equation*}
    (E_m - E_0 )a_j = 
    \begin{cases}
        2J(2a_j -a_{j-1} - a_{j+1}) & 2 \leq j \leq N-1\\
        2J(2a_j -a_{j\pm 1}) & j = (1,2)\\
    \end{cases}
\end{equation*}
with $a_0 = a_{N+1} = 0$. 
...
\begin{equation*}
    a_{j-1} + a_{j+1} = \left( 2 - \frac{E_m }{2J} \right) a_j  = \lambda a_j
\end{equation*}
along with $E_0 = 0$.
...
From this we require $|\lambda|\geq 2$ or that it lies within $\lambda = [-2,2]$.
We can the write $\lambda=2 \cos k$ since it bound the same region, and 
\begin{equation*}
...
\begin{align*}
    E_m a_2 & = 2J (a_1-a_2)\\
    a_2 & = \left( 1- \frac{E_m }{2J }  \right) a_1 = (2 \cos  k -1)a_1\\
    Ae ^{2ik } +B e ^{-2ik} & =  (e ^{ik } + e ^{-ik}-1) (Ae ^{ik } +B e ^{-ik})\\
    & = A(e ^{2ik } + 1 - e ^{ik}) + B(1+ e ^{-2ik } - e ^{-ik})\\
...
\begin{equation*}
    \ket{\tilde{m}  } = 
    \sum_{j } a_m \cos \left[ \frac{\pi }{2 N }(m-1 )(2j-1) \right] \ket{\mathbf{j}}    
\end{equation*}
...
\begin{align*}
   ...
   & =\Re \left(  e^{-i\kappa } e^{2i\kappa } \frac{1 - (e^{2i\kappa })^{N}}{1 - e^{2i\kappa }} \right) \\
    ...
    \sum_{j } ^{N } \cos [\kappa(2j-1)] & =  \Re \left(  e^{-i\kappa } r \frac{1 -  1}{1 - e^{2i\kappa }}  \right) = 0
\end{align*}

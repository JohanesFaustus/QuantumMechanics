...
\end{equation*}
The zero-point energy of the electromagnetic ﬁeld in a region of volume $V$ can be written as
\begin{equation*}
  E_\text{vac}=
...
In addition to the total energy there are other interesting conserved quantities which characterize the classical radiation ﬁeld.
...
\begin{equation*}
  \frac{dc_{n}}{dt} = i [H, c_{n}] = -i E_{n} c_{n}
\end{equation*}

\subsubsection{Derivation: expectation of Hamiltonian.}
...
\begin{align*}
  \braket{H }
   & =  \int d^3 r \left( \sum_{m} a_m^*(t) \psi_m^*(r) \right) H \left( \sum_{n} a_n(t) \psi_n(r) \right) \\
   & =  \sum_{m,n} a_m^*(t) a_n(t) \int d^3 r \, \psi_m^*(r) H \psi_n(r)                                   \\
   & = \sum_{m,n} a^*_m(t) a_n(t) E_n \int d^3 r \, \psi^*_m(r) \psi_n(r)                                  \\
   & = \sum_{m,n} a^*_m(t) a_n(t) E_n \delta_{mn}                                                          \\
  \braket{H }
   & = \sum_{n}  a_n(t) a^*_n(t) E_n
\end{align*}
At this step, both boson and fermion produce the same result.
The distinction appears when one performs the operator promotion.

\subsubsection{Derivation: Heisenberg equation.}
Compute first the commutator of Hamilton with boson annihilation operator
\begin{align*}
  [H,a_n] & = \sum_{m} E_m  [a_m^\dagger a_m, a_n]                                              \\
          & = \sum_{m } E_m \left(     a_m^\dagger [a_m, a_n] + [a_m^\dagger, a_n] a_m. \right) \\
  [H, a_n]
          & = -\sum_{m }E_m \delta_{mn} a_m= -E_n a_n                                           \\
\end{align*}
then with fermion annihilation
\begin{align*}
  [H, c_n] & = \sum_m E_m [c_m^{\dagger} c_m, c_n]                                           \\
           & =  \sum_m E_m (c_m^{\dagger} c_m c_n - c_n c_m^{\dagger} c_m)                   \\
           & =  \sum_m E_m (c_m^{\dagger} c_m c_n - \delta_{mn} c_m + c_m^{\dagger} c_n c_m) \\
  [H, c_n] & =  -\sum_m E_m \delta_{mn} c_m =  -E_n c_n                                      \\
\end{align*}
\end{document}
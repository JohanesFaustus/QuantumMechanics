...
\end{align*}
where $0_F$ is the zero of the Fock space (usually indicated with $0$), and $| \mathbf{k} s \rangle$ is clearly the state of one photon with wavevector $k$ and polarization $s$, such that
\begin{equation*}
...
For Fermion, the action of creation and annihilation operators are as follows
\begin{align*}
  \hat{c}_{i} | \cdots n_{i} \cdots \rangle         & = (-1)^{\nu_i}\sqrt{n_{i}} |  \cdots n_{i} - 1 \cdots \rangle    \\
  \hat{c}^\dagger_{i} | \cdots n_{i} \cdots \rangle & = (-1)^{\nu_i} \sqrt{ 1 - n_i} |\cdots  n_{i} + 1 \cdots \rangle
\end{align*}
with $n_i \equiv n_{\mathbf{k }s} \in \{0,1\}$ and $(-1)^{\nu_i}$ being the phase factor.
The phase factor is defined in a way such that if levels that are occupied by a Fermion $n_i$ is even for the levels which are predecessors of the level $i$, then the phase is $(+1)$, and if it is odd, then put $(-1)$
\begin{equation*}
  \nu_{i} =  \sum_{j=1}^{i -1} n_j
\end{equation*}
This definition also enforce the Pauli exclusion principle
\begin{equation*}
  (\hat{c}_{i})^2 | \cdots n_{i} \cdots \rangle= 0\qquad2  (\hat{c}^\dagger_{i})^2 | \cdots n_{i} \cdots \rangle = 0
\end{equation*}
...
Boson and fermion annihilation and creation operator obey commutator and anti-commutator relation separately 
\begin{equation*}
  \left[a_i,a_j ^\dagger\right] = \delta_{ij},\qquad
  \left\{ c_i,c_j  ^\dagger\right\} = \delta_{ij} 
\end{equation*}
Now consider the case of $i \neq j$.
Boson obey symmetrization, so $a_ia_j=a_ja_i$; fermion in other hand obey antisymmetrization, so $a_ia_j=-a_ja_i$.
Other useful commutator relation include
\begin{equation*}
 [N,\hat{a}_{\mathbf{k }s} ] = - \hat{a}_{\mathbf{k }s} \qquad
 [N,\hat{a}_{\mathbf{k }s} ^\dagger] = \hat{a}_{\mathbf{k }s} ^\dagger
\end{equation*}

\subsubsection{Fermion phase factor.}
To see how the phase factor come to be, consider the Fock state of fermion
\begin{equation*}
|n_1,n_2,\dots\rangle = (c_1^\dagger)^{n_1}(c_2^\dagger)^{n_2}(c_3^\dagger)^{n_3}\dots|0\rangle  
\end{equation*}
Suppose the annihilation operator acts on this state
\begin{equation*}
  c_i|n_1,n_2,\dots\rangle = c_i (c_1^\dagger)^{n_1}(c_2^\dagger)^{n_2}\dots(c_i^\dagger)^{n_i}\dots|0\rangle
\end{equation*}
The operator must eliminate the particle created by $c_i ^\dagger$.
However, between them them lie all earlier factors
\begin{equation*}
  (c_1 ^\dagger)^{n_1}, \ldots, (c_{i-1}^\dagger)^{n_{i-1}}
\end{equation*}
Because fermionic operators satisfy antisymmetry, each $c_j ^\dagger$ introduce factor $(-1)$.


\subsubsection{Bogoliubov transformation.}
The Bogoliubov transformation is to diagonalize quadratic Hamiltonian that mix creation and annihilation operators.
The general quadratic form for boson can be written as
\begin{equation*}
  H = \sum_\mathbf{k} \left( A_\mathbf{k} a_\mathbf{k}^{\dagger} a_\mathbf{k} + \frac{1}{2} B_\mathbf{k} a_\mathbf{k}^{\dagger} a_{-\mathbf{k}} + \frac{1}{2} B_\mathbf{k}^* a_{-\mathbf{k}}^{\dagger} a_\mathbf{k} \right) 
\end{equation*}
We introduce the Bogoliubov transformation for boson
\begin{equation*}
b_\mathbf{k} = u_\mathbf{k}a_\mathbf{k} + v_\mathbf{k}a^{\dagger}_\mathbf{k}
\end{equation*}
which satisfy $|u_\mathbf{k}|^2 - |v_\mathbf{k}|^2 = 1$.
The Hamiltonian become diagonal in this transformation
\begin{equation*}
  H = \sum_\mathbf{k} E_\mathbf{k} \left( b_\mathbf{k}^\dagger b_\mathbf{k} + \frac{1}{2} \right) - \frac{1}{2} \sum_\mathbf{k} A_\mathbf{k}
\end{equation*}
with $E_\mathbf{k}=\sqrt{A_\mathbf{k}^2-B_\mathbf{k}^2}$

For fermions the quadratic Hamiltonian with pairing terms has the form
\begin{equation*}
  H = \sum_\mathbf{k} \left( \epsilon_\mathbf{k} c_\mathbf{k}^\dagger c_\mathbf{k} + \Delta_\mathbf{k} c_\mathbf{k}^\dagger c_{-\mathbf{k}}^\dagger + \Delta_\mathbf{k}^* c_{-\mathbf{k}} c_\mathbf{k} \right) 
\end{equation*}
The fermionic Bogoliubov transformation reads
\begin{equation*}
  \gamma_\mathbf{k} = u_\mathbf{k} c_\mathbf{k} + v_\mathbf{k} c_\mathbf{k}^\dagger
\end{equation*}
which satisfy $u_\mathbf{k}|^2 - |v_\mathbf{k}|^2 = 1$.
Hamiltonian then becomes diagonal
\begin{equation*}
  H = \sum_\mathbf{k} E_\mathbf{k} \gamma_\mathbf{k}^\dagger \gamma_\mathbf{k} + E_0
\end{equation*}
...
\begin{align*}
  ...
  \hat{N} \left( \hat{a}^\dagger_{\mathbf{k }s} \ket{n_{\mathbf{k }s}} \right) & =  (n_{\mathbf{k }s}+1) \left( \hat{a}^\dagger_{\mathbf{k }s} \ket{n_{\mathbf{k }s}} \right)
\end{align*}
...
Next is to find the coefﬁcients
\begin{equation*}
...
\end{equation*}
For the annihilation coefﬁcients
\begin{equation*}
...
\subsubsection{Derivation: Bogoliubov transformation.}
We derive the condition at which the commutation relation for boson and anti-commutator for fermions is preserved.
First by the transformation 
\begin{equation*}
b = ua + va^\dagger\qquad
b^\dagger = u^* a^\dagger + v^* a
\end{equation*}
The commutation relation must hold true
\begin{align*}
[b, b^\dagger] 
& =  [ua + va^\dagger, u^* a + v^* a]\\
& =  |u|^2[a, a^\dagger] + uv^*[a, a] + vu^*[a^\dagger, a^\dagger] + |v|^2[a^\dagger, a]\\
|u|^2-|v|^2 & =  1
\end{align*}
For fermions, the transformation reads 
\begin{equation*}
\gamma = u c + v c^\dagger\qquad
\gamma^\dagger = u^* c^\dagger + v^* c
\end{equation*}
Then the anti-commutator
\begin{align*}
\{ \gamma, \gamma^\dagger \} 
& =  \{ u c + v c^\dagger, u^* c^\dagger + v^* c \}\\
& =  |u|^2 \{ c, c^\dagger \} + uv^* \{ c, c \} + v u^* \{ c^\dagger, c^\dagger \} + |v|^2 \{ c^\dagger, c \} \\
  |u|^2 + |v|^2 & = 1
\end{align*}
...
\subsection{Quantum Field Operator}
The quantum field operators  annihilate and create, respectively, a particle at position $\mathbf{r}$
\begin{equation*}
  \psi^{\dagger}(\mathbf{r})|0\rangle = |\mathbf{r}\rangle\qquad
  \psi(\mathbf{r})|\mathbf{r}'\rangle = \delta(\mathbf{r} - \mathbf{r}')|0\rangle
\end{equation*}
The field operator defined as transformation from single particle basis into position basis of the annihilation and creation operator
\begin{align*}
  \psi(\mathbf{r})           & =  \sum_{\alpha} \langle \mathbf{r} | 1_ \alpha \rangle a_{\alpha} = \sum_{\alpha} u_{\alpha}(\mathbf{r}) a_{\alpha}                        \\
  \psi^{\dagger}(\mathbf{r}) & =  \sum_{\alpha} \langle 1_\alpha | \mathbf{r} \rangle a_{\alpha}^{\dagger} = \sum_{\alpha} u_{\alpha}^{*}(\mathbf{r}) a_{\alpha}^{\dagger}
\end{align*}
where $u_\alpha(\mathbf{r})$ is the wave function of the one-particle state $\ket{1_\alpha}$.
With this in mind, the position eigenstate reads 
\begin{equation*}
  \ket{\mathbf{r}} 
  =  \sum_{\alpha} \langle 1_\alpha | \mathbf{r} \rangle a_{\alpha}^{\dagger} \ket{0} 
  = \sum_{\alpha} u_{\alpha}^{*}(\mathbf{r}) a_{\alpha}^{\dagger} \ket{0}
\end{equation*}
Meanwhile, the inverse transformation are
\begin{align*}
  a_{\alpha}           & =  \int d^3 r \; \langle 1_{\alpha} | \mathbf{r} \rangle \psi_{\sigma} (\mathbf{r}) = \int d^3 r \, u_{\alpha}^{*} (\mathbf{r}) \psi (\mathbf{r})       \\
  a_{\alpha}^{\dagger} & =  \int d^3 r \; \langle\mathbf{r} | 1_{\alpha} \rangle \psi^{\dagger} (\mathbf{r}) = \int d^3 r \, u_{\alpha} (\mathbf{r}) \psi^{\dagger} (\mathbf{r})
\end{align*}

The field operators obey similar commutator relation as the creation and annihilation counterpart
\begin{equation*}
 [\psi(\mathbf{r }), \psi^{\dagger}(\mathbf{r}')] = \delta(\mathbf{r }-\mathbf{r}') \qquad
 \{\psi(\mathbf{r }), \psi^{\dagger}(\mathbf{r}')\} = \delta(\mathbf{r }-\mathbf{r}')  
\end{equation*}
where boson obey the commutator relation while fermion obey the anticommutation.

As with the case of number operator for annihilation and creation, the number of particle in a volume $V$ is
\begin{equation*}
  N
  = \int_V d^3 r \; \psi^\dagger(\mathbf{r}) \psi(\mathbf{r})
  = \int_V d^3 r \; \rho(\mathbf{r})
\end{equation*}

In the case of free particle in a box, we can evaluate $u_\alpha(\mathbf{r})$.
The final result for creation and annihilation operators is then  given by
\begin{equation*}
  a_\mathbf{k} = \int d^3 r \, e^{-i \mathbf{k} \cdot \mathbf{r}} \psi(\mathbf{r})\qquad
  a_\mathbf{k}^{\dagger} = \int d^3 r \, e^{i \mathbf{k} \cdot \mathbf{r}} \psi^{\dagger}(\mathbf{r})
\end{equation*}
and its inverse
\begin{equation*}
  \psi(\mathbf{r}) = \frac{1}{V} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{r}} a_\mathbf{k}\qquad
  \psi^{\dagger}(\mathbf{r}) = \frac{1}{V} \sum_\mathbf{k} e^{-i \mathbf{k} \cdot \mathbf{r}} a_\mathbf{k}^{\dagger}
\end{equation*}

\subsubsection{Derivation: field operator action}
We consider the action of 
\begin{align*}
\hat{\psi} ^\dagger (\mathbf{r},t)\lvert 0\rangle 
&= \sum_\alpha \hat{c}^\dagger_\alpha(t)\,\phi^*_\alpha(\mathbf{r})\lvert 0\rangle \\
& =  \sum_\alpha \hat{c}^\dagger_\alpha e^{i\epsilon_\alpha t/\hbar}\langle \alpha|\mathbf{r}\rangle\lvert 0\rangle \\
&= \sum_\alpha e^{i\epsilon_\alpha t/\hbar}\lvert \alpha\rangle\langle \alpha|\mathbf{r}\rangle \\
&= \sum_{\alpha} e^{i H t/\hbar} \lvert \alpha\rangle\langle \alpha|\mathbf{r}\rangle \\
\hat{\psi} ^\dagger (\mathbf{r},t)\lvert 0\rangle 
& =  e^{i H t/\hbar}\ket{\mathbf{r}} = \ket{\mathbf{r },t}\\
\end{align*}
and
\begin{align*}
\hat{\psi}  (\mathbf{r},t)\lvert \mathbf{r},t\rangle 
&= \sum_{\alpha\beta} \hat{c}_\alpha(t)\,\phi_\alpha(\mathbf{r})\hat{c}_\beta ^\dagger(t) \ket{0}\\
&= \sum_{\alpha\beta} \phi_\alpha(\mathbf{r})\phi_\beta^*(\mathbf{r })e ^{-i \epsilon_\alpha t/\hbar }  e^{i \epsilon_\beta t/\hbar} \hat{c}_\alpha  \hat{c}_\beta ^\dagger \ket{0}\\
\hat{\psi} ^\dagger (\mathbf{r},t)\lvert \mathbf{r }, t \rangle  
&= \sum_{\alpha\beta} \phi_\alpha(\mathbf{r})\phi_\beta^*(\mathbf{r })e ^{i \epsilon_\alpha t/\hbar }  e^{i \epsilon_\beta t/\hbar} (\delta_{\alpha\beta} - \hat{c }^\dagger_\beta \hat{c}_\alpha)\ket{0}
= \ket{0}
\end{align*}

\subsubsection{Derivation: field operator transformation}
Start with basis transform of two orthonormal basis
\begin{equation*}
  \ket{1_\alpha} = \sum_{i} \ket{1_i  }\braket{1_i  | 1_\alpha}
\end{equation*}
From the action of annihilation operator $a_i \ket{1_i}=\ket{0}$, we have
\begin{equation*}
  a_i \ket{1_i} = a_\alpha \ket{1_\alpha} = a_\alpha  \sum_{i} \braket{1_i | 1_\alpha } \ket{1_i}
\end{equation*}
We can surmise that the transformation of creation operator is
\begin{equation*}
  a_i = \sum_{i} \braket{1_i | 1_\alpha } a_\alpha
\end{equation*}
The transformation of creation operator can be obtained by taking the adjoint.
We introduce quantum field operators for annihilating particles at position $\mathbf{r}$ by considering position basis, which satisfy
\begin{equation*}
  \langle \mathbf{r} | \mathbf{r}' \rangle = \delta(\mathbf{r} - \mathbf{r}') \qquad
  \int d^3 r \, |\mathbf{r}\rangle \langle \mathbf{r}| = \mathbf{1}
\end{equation*}
Using the transformation into position basis
\begin{equation*}
  \psi(\mathbf{r})          = \sum_\alpha \langle \mathbf{r} | 1_\alpha \rangle a_\alpha = \sum_\alpha u_\alpha(\mathbf{r}) a_\alpha
\end{equation*}
The inverse transformation can be obtained by $\langle 1_\alpha | \mathbf{r} \rangle$ and integrate over $\mathbf{r}$
\begin{align*}
  \int d^3 r \; \langle 1_\alpha | \mathbf{r} \rangle \psi(\mathbf{r})
   & =  \int d^3 r \, \sum_\beta \braket{1_\alpha  | \mathbf{r}}\langle \mathbf{r} | 1_\beta \rangle  a_\beta \\
  \int d^3 r \; \langle 1_\alpha | \mathbf{r} \rangle \psi(\mathbf{r})
   & =  \sum_\beta a_\beta \langle 1_\alpha | 1_\beta \rangle
  = \sum_\beta a_\beta \delta_{\alpha \beta}
  = a_\alpha
\end{align*}

For the case of free particle in a box, it is convenient to work in momentum eigenstate, defined as
\begin{equation*}
  \mathbf{p } \ket{\mathbf{p }} = \mathbf{k } | \mathbf{k} \rangle\qquad
  \sum_{\mathbf{k}} | \mathbf{k} \rangle \langle \mathbf{k} | = \mathbf{1} \qquad
  \langle \mathbf{k} | \mathbf{k}' \rangle = \delta_{\mathbf{k}\mathbf{k}'}
\end{equation*}
The wavefunction in position representation is $\braket{\mathbf{r }|\mathbf{k }  } = e ^{i \mathbf{k }\cdot \mathbf{r}}$, so that the transformation and its inverse 1reads
\begin{equation*}
  a_\mathbf{k} = \int d^3 r \, e^{-i \mathbf{k} \cdot \mathbf{r}} \psi(\mathbf{r})\qquad
  \psi(\mathbf{r}) = \frac{1}{V} \sum_\mathbf{k} e^{i \mathbf{k} \cdot \mathbf{r}} a_\mathbf{k}
\end{equation*}

\subsubsection{Derivation: field operator commutator relation}
We use the annihilation and creation transformation to derive the relation
\begin{align*}
  [\psi(\mathbf{r}), \psi^{\dagger}(\mathbf{r}')] 
  & =  \left[ \sum_{\alpha} u_{\alpha}(\mathbf{r})a_{\alpha}, \sum_{\beta} u_{\beta}^{*}(\mathbf{r}')a_{\beta}^{\dagger} \right] \\
  & = \sum_{\alpha \beta} u_{\alpha}(\mathbf{r})u_{\beta}^{*}(\mathbf{r}')\left[a_{\alpha}, a_{\beta}^{\dagger}\right]\\
  & = \sum_{\alpha \beta} u_{\alpha}(\mathbf{r})u_{\beta}^{*}(\mathbf{r}')\delta_{\alpha \beta} \\
  & = \sum_{\alpha } u_{\alpha}(\mathbf{r})u_{\alpha}^{*}(\mathbf{r}') \\
  [\psi(\mathbf{r}), \psi^{\dagger}(\mathbf{r}')] 
  & = \sum_{\alpha } \braket{\mathbf{r }  | 1_\alpha }\braket{1_\alpha  | \mathbf{r}'} = \delta (\mathbf{r }-\mathbf{r})
\end{align*}
Same goes for the anticommutator
\begin{align*}
\{\psi(\mathbf{r}),\psi^\dagger(\mathbf{r}')\} 
&= \left\{\sum_\alpha u_\alpha(\mathbf{r})c_\alpha,\ \sum_\beta u^*_\beta(\mathbf{r}')c^\dagger_\beta\right\}\\
&= \sum_{\alpha\beta} u_\alpha(\mathbf{r})u^*_\beta(\mathbf{r}')\{c_\alpha,c^\dagger_\beta\}\\
&= \sum_{\alpha\beta} u_\alpha(\mathbf{r})u^*_\beta(\mathbf{r}')\delta_{\alpha\beta}\\
&= \sum_\alpha u_\alpha(\mathbf{r})u^*_\alpha(\mathbf{r}')\\
\{\psi(\mathbf{r}),\psi^\dagger(\mathbf{r}')\} 
&= \sum_\alpha \langle \mathbf{r}|1_\alpha\rangle\langle1_\alpha|\mathbf{r}'\rangle = \delta(\mathbf{r}-\mathbf{r}')
\end{align*}
\end{document}